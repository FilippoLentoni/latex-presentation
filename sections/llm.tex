...
% ---------------- Slide: Overview ----------------
\begin{frame}{Classic automatic metrics (text generation)}
\begin{itemize}
  \item \textbf{BLEU}: precision-oriented n-gram overlap (common in translation).
  \item \textbf{ROUGE}: recall-oriented overlap (common in summarization).
  \item \textbf{METEOR}: adds synonymy + ordering sensitivity; balances precision/recall.
  \item \textbf{BERTScore}: semantic similarity using contextual embeddings (handles paraphrase).
\end{itemize}
\end{frame}

% ---------------- Slide: BLEU Example 1 ----------------
\begin{frame}{BLEU (example)}
\begin{block}{Expected}
\textcolor{red}{The golden dart frog} is \textcolor{red}{the most poisonous animal on the planet}.
\end{block}

\begin{block}{LLM output}
\textcolor{red}{The most poisonous animal on the planet} is the golden poison frog, also known as \textcolor{red}{the golden dart frog} or golden poison arrow frog.
\end{block}

\begin{itemize}
  \item BLEU tends to reward \textbf{exact phrase overlap} (n-grams).
  \item Extra correct details may not help much if phrasing diverges.
\end{itemize}
\end{frame}
...
% ---------------- Slide: ROUGE Example ----------------
% \begin{frame}{ROUGE (example)}
% \begin{block}{Expected}
% The golden dart frog is the most poisonous animal on the planet.
% \end{block}
...
% \begin{itemize}
%   \item ROUGE is widely used for summarization because it emphasizes \textbf{coverage}.
%   \item Intuition: ``Did the model include the key reference content?''
% \end{itemize}
% \end{frame}

% ---------------- Slide: METEOR Example ----------------
% \begin{frame}{METEOR (example)}
% \begin{block}{Expected}
% The golden dart frog is the most poisonous animal on the planet.
% \end{block}
...
% \begin{itemize}
%   \item METEOR is more forgiving about \textbf{word variants / synonyms}.
%   \item It also considers \textbf{word order differences} (penalizes major re-orderings).
%   \item Useful when you want overlap, but with \textbf{linguistic flexibility}.
% \end{itemize}
% \end{frame}

% ---------------- Slide: BERTScore Example ----------------
\begin{frame}{BERTScore (example)}
\begin{block}{Expected}
The \textcolor{red}{golden dart frog} is the \textcolor{red}{most poisonous animal on the planet}.
\end{block}

\begin{block}{LLM output}
The golden dart frog is one of the \textcolor{red}{most dangerous animals on the planet} due to its \textcolor{red}{high toxicity}.
\end{block}

\begin{itemize}
  \item Captures \textbf{semantic similarity} even when wording changes.
  \item Recognizes paraphrases (e.g., \emph{poisonous} $\leftrightarrow$ \emph{high toxicity}).
  \item Often closer to human judgment for meaning, but can still miss factual errors.
\end{itemize}
\end{frame}

% % ---------------- Slide: Key takeaway ----------------
% \begin{frame}{Takeaway: what these metrics miss}
% \begin{itemize}
%   \item They are \textbf{cheap and scalable}, but can be misaligned with real goals.
%   \item Overlap metrics can reward \textbf{copying} more than \textbf{correct reasoning}.
%   \item Embedding metrics capture meaning, but do not guarantee \textbf{factuality}.
%   \item For production: pair with \textbf{task success}, \textbf{human rubrics}, and \textbf{trace-based evaluation}.
% \end{itemize}
% \end{frame}
